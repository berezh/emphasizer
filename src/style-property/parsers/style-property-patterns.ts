...
function completeBoxShadow2(): string {
  // 1px 1px black
  return wrapStartEnd(join(boxShadowInset, dimention(), dimention(), colorPattern.colorPatternString, boxShadowInset, wrapOptional(semicolon())));
}

function completeBoxShadow3(): string {
  // 1px 1px 1px black
  return wrapStartEnd(
    join(boxShadowInset, dimention(), dimention(), dimention(), colorPattern.colorPatternString, boxShadowInset, wrapOptional(semicolon()))
  );
}
